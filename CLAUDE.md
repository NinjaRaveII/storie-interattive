...
>
> *Versione 3.14 — 26 agosto 2026. **4ª storia: «La Nave delle Stelle Sommerse», regno dell'Oceano sbloccato + nuovo pattern «3 immagini di finale».** Su richiesta di Alberto, aggiunta la prima storia del regno **L'Oceano Profondo** (🌊, id `sea`, prima bloccato con `storyIds:[]`): protagonista **Christian**, un bambino che segue la ninnananna di una conchiglia dorata fino alla leggendaria Nave delle Stelle, con tre compagni — Nerea (tartaruga marina), Vela (narvalo), Rubino (polpo timido) — uno per ramo. Quarto genere in catalogo: "Mistero gentile · bambini" (dopo Avventura/`oasis`, Fiaba/`bell`, Buonanotte/`firefly`). **Novità architetturale, non solo contenuto:** Alberto ha chiesto che questa storia abbia **3 immagini di finale invece di 1** (`end_dive.jpg`/`end_boat.jpg`/`end_cave.jpg`, una per ramo della prima scelta, sullo stesso principio delle `middle_<scelta1>.jpg`) al posto della `end.jpg` unica condivisa dai 9 finali (§5, §9 punto 8, decisa in v3.4 e mai più cambiata). **Scelta implementativa chiave: fallback, non breaking change.** `imgPath(id,'end',variant)` (`tv.html`) ora ritorna `end_<variant>.jpg` se `variant` è passato, altrimenti `end.jpg`; `renderStep()` passa sempre `choice1`. `loadImg()` ha guadagnato un secondo parametro `fallbackPath`: se `end_<c1>.jpg` dà 404, ricarica automaticamente `end.jpg`. Risultato: `oasis`/`bell`/`firefly` (che hanno solo `end.jpg`) **continuano a funzionare senza toccare i loro asset** — verificato in browser: la storia `oasis` con `pick('dune',0);pick('dig',1)` carica correttamente `images/oasis/end.jpg` passando comunque per il ramo `end_dune.jpg` (404 silenzioso) prima del fallback. Il pattern a 3 immagini diventa così lo **standard consigliato per le storie future**, senza bisogno di rifare le 3 storie esistenti. Aggiunto anche un `onerror` sulle `<img class="m-img">` della galleria medaglie (nasconde l'icona rotta finché l'asset non esiste — utile ora che `story:tide` e `realm:sea` sono comparsi nel medagliere dinamico prima di avere i loro PNG). **Dati e audio generati in questa sessione:** blocco storia completo in `stories.js` (9 finali, tutti con testo+morale, nessuna scelta punitiva), `REALMS.sea.storyIds:['tide']`, cache-buster `stories.js?v=7→v=8` (in `tv.html` e `controller.html`); i 13 file audio (`audio/tide/`) generati con `node generate-audio.mjs` (nessuna modifica allo script: deriva tutto da `stories.js`, come da design). Ricreato anche `scratchpad/validate.mjs` (citato da `TEST_CHECKLIST.md` ma mai committato, §13 lezione già nota). **✅ Immagini generate in autonomia nella stessa sessione**, su richiesta esplicita di Alberto ("usa la tua estensione Chrome, come già fatto per gli altri regni"): nessun tool/chiave API di generazione immagini è disponibile via MCP (verificato: `.env.local` ha solo `ELEVENLABS_API_KEY`), quindi Claude Code ha pilotato **Gemini** nel Chrome reale di Alberto tramite l'estensione **Claude in Chrome** — stessa unica conversazione per tutte le 9 immagini (7 scene panoramiche 1856×576 + 2 ritratti su sfondo bianco), per la coerenza del personaggio Christian richiesta da `GUIDA_STORIE.md` §6. Le 3 immagini generate con proporzioni diverse dalle altre sono state ricondotte a 1856×576 con un ritaglio centrato via Pillow (nessuna generazione persa). Per i 2 ritratti medaglia (`tide.png`, `realm_sea.png`), `rembg` si è rivelato **non utilizzabile su questa macchina**: l'import fallisce con *"An Application Control policy has blocked this file"* su più DLL compilate a catena (`scikit-image`→`scipy`→`numba`), anche dopo reinstallazione pulita — non un pacchetto corrotto ma un vero blocco di sicurezza di sistema (verosimilmente Kaspersky Application Control), quindi **non aggirato** con permessi/exclusion (fuori dallo scope di Claude Code). Soluzione: bypassata l'importazione del pacchetto `rembg` (che trascina scipy/skimage/numba) e reimplementato lo scontorno **a mano** con solo `onnxruntime` + `numpy` + `PIL`, chiamando direttamente il modello `u2net.onnx` già in cache locale (`~/.u2net/`, scaricato in una sessione precedente) — stesso identico modello usato da rembg, stesso risultato visivo, senza le dipendenze bloccate. Per il ritratto di Christian, dopo due tentativi di generazione "full body" andati storti (Gemini generava solo primi piani/busto nonostante il prompt "FULL-BODY", con generazioni che restavano bloccate a metà finché non interrotte manualmente), un terzo tentativo con prompt più diretto ("FULL BODY from head to bare feet, standing straight") ha prodotto un risultato completo e pulito. Verificato in preview: **110/110 check** in `validate.mjs` (0 warning, 0 errori), `images/tide/intro.jpg` e `end_dive.jpg` caricati correttamente (`naturalWidth` confermato via JS), medaglie `story:tide`/`realm:sea` visibili nel medagliere (non più nascoste dall'`onerror`).*
>
> *Versione 3.13 — 10 luglio 2026. **Medaglia «La prima storia» sparita di nuovo — questa volta era l'animazione, non l'immagine.** Segnalato da Alberto: la medaglia tornava invisibile nel medagliere, sintomo identico al bug v3.11 ma **senza che l'asset fosse stato toccato** (`images/medals/first_story.png` invariato in git dalla v3.11, verificato pixel-per-pixel: bordi trasparenti, nessuno sfondo nero — il file non è la causa). **Causa reale:** `.medal-item.complete .m-img` aveva ancora `animation:m-glow …infinite`, che interpola ogni 2,6s tra due catene di `filter:drop-shadow()…drop-shadow()` diverse — esattamente il pattern che il changelog v3.11 dichiarava di aver eliminato ("rimosse… le animazioni di filter, non affidabili sui browser TV") ma che in realtà era rimasto **solo** sullo stato "completa/accesa". «La prima storia» è la medaglia che raggiunge quello stato per prima in assoluto (si accende al *primo* finale visto da chiunque), quindi è il primo punto in cui l'app di ogni famiglia incontra il pattern rischioso — coerente col fatto che il bug si sia "ripresentato" senza alcuna modifica all'immagine. **Fix (solo `tv.html`, § CSS medaglie):** rimossa `animation:m-glow` e il relativo `@keyframes`; lo stato "completa" ora ha un **filtro `drop-shadow` statico** (calcolato una sola volta, nessun repaint continuo) leggermente più luminoso della vecchia base, così l'alone dorato resta ben visibile senza dipendere da un'interpolazione di `filter` per sempre in loop. Lasciate intatte le altre animazioni della galleria (`m-bob` su `transform`, il luccichio `m-tw` su `opacity`+`transform`, `fadeIn` su `opacity`) perché usano proprietà già collaudate sulla stessa pagina (lo stato "esplorata", che condivide `m-bob`, non è mai stato segnalato come invisibile). Verificato in Chrome via preview (720p): simulato un finale in `localStorage` per forzare lo stato "completa", screenshot prima/dopo, ispezione dello stile calcolato (`animation-name` assente, `filter` statico), console pulita. ⚠️ **Non riproducibile in preview headless** (il bug è specifico del motore CSS della smart TV, non di Chrome): resta da confermare sulla **TV Samsung reale** che la medaglia rimanga visibile stavolta — nessun'altra medaglia "completa" era ancora stata raggiunta in famiglia, quindi anche questo era un rischio latente non ancora osservato per le altre. **Lezione (aggiunta anche in §13):** quando un changelog dichiara "rimossa l'animazione di filter", verificare la regola CSS specifica per **ogni stato**, non fidarsi della frase riassuntiva — la rimozione era stata fatta per `locked`/`explored` ma dimenticata per `complete`, l'unico stato dove l'effetto (l'alone "acceso") sembrava intenzionalmente valere la pena del rischio.*
>
> *Versione 3.12 — 8 luglio 2026. **Guida PDF per gli utenti finali (solo locale, nessun cambio al sito).** Creato `Istruzioni-Famiglia.pdf` (5 pagine, ~1MB), un documento grafico stile "libro illustrato" (stesso tema dark fantasy dell'app: oro, Playfair Display/Crimson Pro) pensato per famiglia e amici — copertina, come iniziare (link corto + preferiti + schermo intero + QR), come si gioca, le medaglie, comandi utili dal telefono e FAQ. Gli screenshot sono reali (catturati con uno script Puppeteer headless puntato sull'app locale, non mockup) e compressi in JPEG per restare inviabile via WhatsApp. **Scelta esplicita dell'utente: il PDF resta solo locale**, non versionato in git né pubblicato su GitHub Pages — nessun impatto su `tv.html`/`controller.html`/`stories.js`/`transport.js`, nessun bump `?v=`. Creato anche un link corto (`tinyurl.com/pierfra` → l'URL GitHub Pages) da digitare una sola volta sul telecomando della TV, poi da salvare nei preferiti. ⚠️ **Nota per chi riprende questo lavoro:** l'HTML sorgente del PDF e lo script di generazione/screenshot sono vissuti solo in una cartella temporanea di sessione (non nel repo) — rigenerare il documento da zero richiede di ricostruirli, non sono recuperabili da qui. **Lezione appresa (aggiunta anche in §13):** nel CSS di stampa, forzare pagine a dimensione esatta (`min-height:297mm` + `page-break-after:always`) è fragile — se anche un solo pixel di contenuto eccede l'altezza fisica della pagina, Chrome aggiunge silenziosamente una pagina bianca extra in coda al PDF, invisibile finché non si conta con un tool (`pdf-lib`) o si misura l'altezza renderizzata di ogni pagina (`getBoundingClientRect`), non basta ispezionare gli screenshot.*
>
...
| `tide` | La Nave delle Stelle Sommerse | Mistero gentile · bambini | L'Oceano Profondo | `dive` / `boat` / `cave` |
...
> ⚠️ **Scelta deliberata, ora con due varianti supportate (dalla v3.14):** lo standard originale (`oasis`/`bell`/`firefly`) resta **un'unica** `end.jpg` neutra/trionfale condivisa dai 9 finali — a differenza dell'audio, dove ogni finale ha testo diverso e quindi *deve* avere un mp3 diverso. Dalla v3.14, una storia può avere invece **3 immagini di finale** (`end_<scelta1>.jpg`, una per ramo, sullo stesso principio di `middle_<scelta1>.jpg` — non ancora le 9 per combinazione completa): è il caso di `tide`. `imgPath()`/`loadImg()` in `tv.html` gestiscono entrambe le varianti con un fallback automatico (`end_<c1>.jpg` → `end.jpg` se il primo dà 404), quindi le storie vecchie non richiedono alcuna modifica ai loro asset. Le 9 immagini di finale per combinazione completa restano un possibile step futuro (Fase 3).
...
20. **✅ Medaglia «La prima storia» sparita di nuovo — causa reale trovata (10 luglio 2026).** Non era l'immagine (invariata dalla v3.11, verificata trasparente): era `animation:m-glow` rimasta sullo stato "completa" (`.medal-item.complete .m-img`), che il changelog v3.11 credeva di aver già eliminato ovunque. Rimossa l'animazione di `filter`, sostituita con un alone dorato **statico**. Vedi dettagli in cima al documento (v3.13) e lezione in §13.15.
21. **✅ 4ª storia (`tide`) completa, testo+dati+audio+immagini (26 agosto 2026).** Vedi changelog v3.14 in cima al documento per tutti i dettagli: nuovo pattern "3 immagini di finale" con fallback (nessuna rottura per le storie esistenti), generazione immagini in autonomia via estensione Claude in Chrome + Gemini, scontorno medaglie senza `rembg` (bloccato da una policy di sicurezza di sistema) sostituendolo con una chiamata diretta a `onnxruntime`+`numpy`+`PIL` sullo stesso modello `u2net.onnx`. Lezione tecnica anche in §13.16.

### Ancora aperti
- **Test su TV Samsung reale del sistema medaglie (galleria + festa).** Introdotto in v3.7, non ancora provato su un browser TV vero: priorità #1 per la prossima sessione hands-on con la TV (checklist in `TEST_CHECKLIST.md` §B4). Include ora anche la verifica del fix v3.13 (filtro statico sulla medaglia "prima storia") e delle altre medaglie "complete" (mai ancora raggiunte in famiglia, stesso rischio latente non osservato), più le 9 nuove immagini di `tide` (mai viste su TV reale).
...
- [x] **Sblocco del regno `sea`** (26 agosto 2026): prima storia del regno L'Oceano Profondo, `tide` — "La Nave delle Stelle Sommerse", dati+audio+**tutte e 9 le immagini** generate e verificate. Restano bloccati `mountain` e `sky`.
- [ ] Nuove storie e sblocco dei regni `mountain` / `sky` — obiettivo catalogo dal documento di strategia: 2 storie per regno (12 totali) + un regno "vetrina" a 3, verso il traguardo 3×6 = 18. Oggi: 4 storie, 4 regni attivi.
...
14. **Pagine a dimensione esatta nel CSS di stampa: verificare l'altezza renderizzata, non solo lo screenshot (8 luglio 2026).** Generando `Istruzioni-Famiglia.pdf` con Puppeteer, ogni "pagina" era un `<div>` con `min-height:297mm` (A4) e `page-break-after:always`. Con quattro immagini impilate, una pagina è risultata ~8mm più alta del fisico A4: l'eccedenza è "traboccata" su una nuova pagina fisica **prima** che il `page-break-after` esplicito scattasse, producendo una **6ª pagina quasi bianca in coda** — invisibile scorrendo gli screenshot delle singole pagine (ognuno sembrava correttamente pieno), visibile solo contando le pagine reali del PDF (`pdf-lib`, `doc.getPageCount()`) o misurando `getBoundingClientRect().height` di ogni `.page` in `page.evaluate()`. Morale: quando il layout è vincolato a una dimensione fisica esatta (mm/pollici), un controllo visivo non basta — serve una misura numerica dell'altezza effettiva contro il limite.
15. **Una frase di changelog che dice "rimossa l'animazione di filter" va verificata regola per regola, non presa per buona (10 luglio 2026).** La v3.11 dichiarava di aver eliminato le animazioni di `filter` dalle medaglie per compatibilità TV (§13.8) — vero per gli stati `locked`/`explored`, ma `.medal-item.complete .m-img` aveva ancora `animation:m-glow` (interpolazione continua tra due catene di `drop-shadow`). Risultato: la medaglia «La prima storia», l'unica a raggiungere lo stato "completa" nei primi minuti d'uso di ogni famiglia, è tornata invisibile — un bug creduto chiuso si è "ripresentato" senza che nessuno avesse toccato l'asset incriminato. Prima di fidarsi di un riepilogo tipo "rimosso X ovunque", grep-are la proprietà nel CSS per **ogni** classe di stato coinvolta. Fix: filtro statico invece che animato per lo stato "accesa" — l'effetto resta (alone dorato più intenso), il rischio di repaint continuo su motori CSS deboli sparisce.
16. **Generazione immagini in autonomia via estensione browser: possibile, ma con caveat pratici (26 agosto 2026).** Pilotare Gemini nel Chrome reale dell'utente tramite l'estensione Claude in Chrome ha funzionato, con alcune scoperte utili per la prossima volta: (a) *finestra minimizzata/in background* → `document.hidden`/dimensioni quasi-zero, gli screenshot vanno in timeout e i click a coordinate falliscono silenziosamente; soluzione robusta: manipolare l'editor (`div.ql-editor[contenteditable]`) e il bottone invio (`button[aria-label="Invia messaggio"]`) **via JS/DOM** (`execCommand('insertText',...)` + `.click()`) invece che via coordinate pixel — funziona anche a finestra non in primo piano, a differenza degli screenshot; (b) *generazioni bloccate a metà* (es. richieste di "FULL-BODY portrait" che restano tagliate alla sola testa) → interrompere (bottone stop) e ripetere con un prompt più diretto risolve quasi sempre, non serve un intervento manuale; (c) *rembg non è scontato che funzioni*: su questa macchina l'import fallisce per un blocco di sicurezza di sistema (Application Control) su `scikit-image`/`scipy`/`numba`, non per un problema di installazione — reinstallare non risolve, e non va forzato/aggirato (è un controllo di sicurezza legittimo dell'utente). Ripiego pulito: chiamare direttamente `onnxruntime` sul modello `u2net.onnx` già in cache (`~/.u2net/`) con pre/post-processing scritto a mano in `numpy`+`PIL` — stesso risultato di `rembg.remove()`, zero dipendenze bloccate. (d) *coerenza personaggio*: generare tutte le immagini nella **stessa conversazione**, richiamando esplicitamente "the exact same X character as before" ha retto bene su 9 immagini di seguito, confermando la pratica già in `GUIDA_STORIE.md` §6.

---

*Documento generato il 30 giugno 2026, ultimo aggiornamento 26 agosto 2026 — versione 3.14.*
