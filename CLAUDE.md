...
- [ ] **Sistema medaglie** *(progettato 5 luglio 2026 — spec completa in `SISTEMA_MEDAGLIE.md` v2.0)*: **7 livelli a piramide** (prima storia; storia esplorata/completa; regno esplorato/completo; tutti i regni esplorati/completati), **derivati da `progress.js`** (nessun dato nuovo salvato). Grafica **2.5D** in CSS (no WebGL/rotazione), tre stati **da conquistare / esplorata / completa (accesa)** — sostituisce sia la vecchia idea "argento/oro" sia il generico "badge di completamento" (per i 3-6 anni conta luce/festa, non il metallo). Forma **«ritaglio sul corpo»**: la medaglia è la sagoma del personaggio vero. **✅ Asset TUTTI fatti (6 luglio 2026)** in `images/medals/`: 3 protagonisti (Sara/Tobia/Bruno, scontornati con `rembg` dalle scene esistenti), 3 landmark dei regni (`realm_forest/kingdom/desert.png`, generati isolati con Gemini nella chat della mappa), livello 1 = **Libro delle Storie** (`first_story.png`); livelli 6/7 riusano `images/map/world.png`. **⏳ Da fare (solo codice):** **galleria in `tv.html`** (bottone sui regni TV, `{action:'medals'}` dal controller, stato "galleria" nella risincronizzazione) + festa a fine storia (`canvas-confetti` + fuochi sugli sblocchi "accesi"). ⚠️ Da testare su TV reale: animazioni CSS (`filter`/`drop-shadow`/`mix-blend-mode`) con ripiego statico.
